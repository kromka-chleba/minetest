...
The following functions are available for both spatial vectors (`vector.*`) and 2D vectors (`vector2.*`).
The behavior is the same for both types, except where noted.
...
* `vector.new([a[, b, c]])` / `vector2.new(x, y)`:
    * Returns a new vector.
    * For 3D: Returns `(a, b, c)`.
    * For 2D: Returns `(x, y)`.
    * Deprecated (3D only): `vector.new()` does the same as `vector.zero()` and
      `vector.new(v)` does the same as `vector.copy(v)`
* `vector.zero()` / `vector2.zero()`:
    * Returns a new zero vector.
    * For 3D: Returns `(0, 0, 0)`.
    * For 2D: Returns `(0, 0)`.
...
* `vector.from_string(s[, init])` / `vector2.from_string(s[, init])`:
    ...
    * For 3D: `s` has to begin with a substring of the form `"(x, y, z)"`.
    * For 2D: `s` has to begin with a substring of the form `"(x, y)"`.
...
* `vector.to_string(v)` / `vector2.to_string(v)`:
    ...
    * For 3D: Returns a string of the form `"(x, y, z)"`.
    * For 2D: Returns a string of the form `"(x, y)"`.
...
* `vector.sort(v1, v2)` / `vector2.sort(v1, v2)`:
    * Returns in order minp, maxp vectors of the cuboid (3D) or rectangle (2D) defined by `v1`, `v2`.

### Angles and products

* `vector.angle(v1, v2)` / `vector2.angle(v1, v2)`:
    * Returns the angle between `v1` and `v2` in radians.
    * For 2D vectors: This is always a positive value (unsigned angle).
* `vector.dot(v1, v2)` / `vector2.dot(v1, v2)`:
    * Returns the dot product of `v1` and `v2`.

### Other operations

* `vector.offset(v, x, y, z)` / `vector2.offset(v, x, y)`:
    * For 3D: Returns the sum of the vectors `v` and `(x, y, z)`.
    * For 2D: Returns the sum of the vectors `v` and `(x, y)`.
...
* `vector.multiply(v, s)` / `vector2.multiply(v, s)`:
    ...
    * Deprecated (3D only): If `s` is a vector: Returns the Schur product.
* `vector.divide(v, s)` / `vector2.divide(v, s)`:
    * Returns a scaled vector.
    * Deprecated (3D only): If `s` is a vector: Returns the Schur quotient.
