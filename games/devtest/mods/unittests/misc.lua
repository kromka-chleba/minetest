...
local function test_get_loaded_active_and_loadable_blocks(_, pos)
	...
	assert(table.indexof(loaded, blockpos) > 0, "expected test block in get_loaded_blocks result")
	assert(table.indexof(active, blockpos) > 0, "expected test block in get_active_blocks result")
end
