...
v3s16 ServerMap::getExtraBlockBorder(u8 target_stage) const
{
	...
	if (target_stage >= STAGE_DECORATIONS && m_emerge) {
		...
		// Ensure vertical overgeneration spans at least one full neighbour
		// chunk so that decorations extending across chunk ceilings/floors
		// can be regenerated by the adjacent chunk (no y-slice clipping).
		border.Y = std::max(border.Y, csize.Y);
	}
	...
	std::map<v3s16, MapBlock*> *changed_blocks, ServerEnvironment *env)
{
	...
	// the central chunk is authoritative. Decoration overgeneration still
	// works because anchors span the full placement area; neighbouring chunks
	// will place and write their own central portions.
