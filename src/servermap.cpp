	...
	std::map<v3s16, MapBlock*> *changed_blocks, ServerEnvironment *env)
{
	...
	/*
		...
		We pass overwrite_generated=false so that border (shell) blocks that
		were already fully generated by a previous mapchunk are not overwritten.
		This prevents adjacent-chunk shell data from clobbering modifications
		made by on_block_loaded callbacks (or any other post-generation edits).
		Center blocks of the current chunk are not yet marked as generated at
		this point, so they are always written regardless of this flag.
	*/
