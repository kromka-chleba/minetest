	...
	std::map<v3s16, MapBlock*> *changed_blocks, ServerEnvironment *env)
{
	...
	*/
	...
	/*
		...
		Use overwrite_generated=false so that shell blocks already fully
		generated by an earlier neighboring chunk are not overwritten with
		the stale snapshot this VManip took at initBlockMake time.
		The central blocks being generated here are still !isGenerated() at
		this point (setGenerated is called further below), so they are always
		written.
	*/
	data->vmanip->blitBackAll(changed_blocks, false, &protected_shell_blocks);
