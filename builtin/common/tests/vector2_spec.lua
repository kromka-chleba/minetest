...
describe("vector2", function()
	...
	describe("angle_to()", function()
		it("returns signed angle from first to second vector", function()
			...
			local a1 = vector2.angle_to(vector2.new(1, 0), vector2.new(0, 1))
			assert.is_true(almost_equal(math.pi / 2, a1))
			
			-- From north to east: negative pi/2 (clockwise)
			local a2 = vector2.angle_to(vector2.new(0, 1), vector2.new(1, 0))
			assert.is_true(almost_equal(-math.pi / 2, a2))
			
			-- From east to west: pi
			local a3 = vector2.angle_to(vector2.new(1, 0), vector2.new(-1, 0))
			assert.is_true(almost_equal(math.pi, math.abs(a3)))
			
			-- Same direction: 0
			local a4 = vector2.angle_to(vector2.new(1, 1), vector2.new(2, 2))
			assert.is_true(almost_equal(0, a4))
		end)

		it("works with method call syntax", function()
			local a = vector2.angle_to(vector2.new(1, 0), vector2.new(0, 1))
			local b = vector2.new(1, 0):angle_to(vector2.new(0, 1))
		...
		it("is opposite of reversed angle", function()
			...
			local a1 = vector2.angle_to(v1, v2)
