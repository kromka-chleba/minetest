...
describe("vector2", function()
	describe("new()", function()
		it("constructs", function()
			assert.same({x = 0, y = 0}, vector2.new())
			assert.same({x = 1, y = 2}, vector2.new(1, 2))
			assert.same({x = 3, y = 2}, vector2.new({x = 3, y = 2}))

			assert.is_true(vector2.check(vector2.new()))
			assert.is_true(vector2.check(vector2.new(1, 2)))
			assert.is_true(vector2.check(vector2.new({x = 3, y = 2})))

			local input = vector2.new({ x = 3, y = 2 })
			local output = vector2.new(input)
	...
	it("zero()", function()
		...
		assert.same(vector2.new(), vector2.zero())
		assert.equal(vector2.new(), vector2.zero())
	...
	it("copy()", function()
		...
		assert.same(vector2.new(v), vector2.copy(v))
		assert.equal(vector2.new(v), vector2.copy(v))
	...
	it("length()", function()
		...
		assert.equal(0, vector2.length(vector2.new()))
	...
	it("normalize()", function()
		...
		assert.equal(vector2.new(), vector2.normalize(vector2.new()))
	end)
	...
	it("dot()", function()
		...
		assert.equal(0, vector2.new():dot(vector2.new(1, 2)))
	end)
	...
	it("multiply()", function()
		local a = vector2.new(1, 2)
		local b = vector2.new(2, 4)
		local c = vector2.new(2, 8)
		local s = 2
		local d = vector2.new(2, 4)
		assert.equal(c, vector2.multiply(a, {x = 2, y = 4}))
		assert.equal(c, vector2.multiply(a, b))
	...
	it("divide()", function()
		local a = vector2.new(1, 2)
		local b = vector2.new(2, 4)
		local c = vector2.new(0.5, 0.5)
		local s = 2
		local d = vector2.new(0.5, 1)
		assert.equal(c, vector2.divide(a, {x = 2, y = 4}))
		assert.equal(c, vector2.divide(a, b))
	...
	it("from_string()", function()
		...
		assert.same({v, 9}, {vector2.from_string(" ( 1, 2) ")})
		assert.same({vector2.new(), 6}, {vector2.from_string("(0,0) ( 1, 2) ")})
